...
class ContentBasedModule(nn.Module):
    ...
    def generate_palm_recommendations(self, paper_id, num_recommendations):
        """
        Generates recommendations using the PaLM API.

        Args:
            paper_id: The ID of the paper to generate recommendations for.
            num_recommendations: The number of recommendations to generate. Make sure it's within [1, 8].

        Returns:
            A list of paper IDs, sorted by their similarity scores.
        """
        try:
            # Get the paper title and abstract
            paper = self.papers_dict.get(paper_id, {})
            paper_title = paper.get('paper title', '')

            if 'abstract' in paper:
                paper_abstract = paper['abstract']
            else:
                # Handle the case where 'abstract' is missing
            ...
            temperature = 0.25
            candidate_count = min(num_recommendations, 8)  # Ensure it's within the valid range
            top_k = 40
            top_p = 0.95
            context = f'You are a paper recommender, and your job is to recommend new papers based on paper title and abstract for paper ID {paper_id}.'
            examples = []
            messages = [{"content": f"{paper_title} {paper_abstract}"}]

            # Call the model and get the response
            response = palm.chat(model=model, temperature=temperature, candidate_count=candidate_count, top_k=top_k, top_p=top_p, context=context, examples=examples, messages=messages)

            recommended_paper_ids = [candidate.get('content', '') for candidate in response.candidates]

            return recommended_paper_ids
        except Exception as e:
            # Handle exceptions (e.g., API errors) and log them
            print(f"Error in generate_palm_recommendations: {e}")
            return []


    def calculate_palm_score(self, paper_id, recommended_paper_id):
        try:
            recommended_paper_ids = self.generate_palm_recommendations(paper_id, num_recommendations=10)
            if recommended_paper_id in recommended_paper_ids:
                rank = recommended_paper_ids.index(recommended_paper_id)
                score = 1.0 - (rank / len(recommended_paper_ids))
            else:
                score = 0.0
            return score
        except Exception as e:
            # Handle exceptions (e.g., no recommendations) and log them
            print(f"Error in calculate_palm_score: {e}")
            return 0.0  # Return a default score

    def forward(self, paper1_id, paper2_id):
        # Calculate content-based similarity between two papers
        content_based_similarity = self.cosine_similarity(self.term_freq(paper1_id), self.term_freq(paper2_id))
        
        # Calculate PaLM similarity score for the two papers
        palm_similarity = self.calculate_palm_score(paper1_id, paper2_id)
        
        # Combine the content-based and PaLM scores
        hybrid_score = (content_based_similarity + palm_similarity) / 2
